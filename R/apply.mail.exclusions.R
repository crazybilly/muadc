#' Apply Mail Exclusoins
#' 
#' @description Apply the standard Millikin mail exclusions to a mailing list.
#'
#' @param df A data frame to be used as a mailing list. 
#' @param idcol A character string describing the name of the id column. Defaults to "pidm". If addr1, deceased, primdnrind and excl columns do not exist in df, they will be joined in from hallptbl using this column.
...
apply.mail.exclusions  <- function(df,  idcol = 'pidm', primary.only = T, include.students = F, sol = F, exclude.fall = F, exclude.spring = F) {
  ...
  mailexclcols      <-  c('addr1','deceased','primdnrind','excl')
